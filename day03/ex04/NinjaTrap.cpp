...
{
    std::cout << "NINJ4-TP default constructor called, " << _name << " ready to play you a trick" << std::endl;
}

NinjaTrap::NinjaTrap(std::string name) : ClapTrap(name, 60, 60, 120, 120, 1, 60, 5, 0)
{
    std::cout << "NINJ4-TP[" << name << "] ready to play you a trick" << std::endl;
}
...
void NinjaTrap::ninjaShoebox(ClapTrap &clp) const
{
    ...
    std::cin >> cmd;
    if (cmd.compare("3") == 0)
        std::cout << "NINJ4-TP[" << this->getName() << "] : Love you too " << clp.getName() << " !!" << std::endl;
    else if (cmd.compare("2") == 0 || cmd.compare("1") == 0)
        std::cout << "NINJ4-TP[" << this->getName() << "] : pfff .. They are idiot daughters !" << std::endl;
    else
        std::cout << "NINJ4-TP[" << this->getName() << "] : you disappoint me, i thought you would be able to admit your preference" << std::endl;
}
